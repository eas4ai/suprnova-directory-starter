...
[OVR-001] The admin overview MUST show actual listing and article publication counts and pending moderation work from the application database, with truthful empty and error states.
Falsifier: A populated directory claims it has no published listings, an expired or suspended listing counts as publicly visible, an unpublished article counts as published, a pending revision is omitted from review work, or a failed query is presented as zero activity.
...
Falsifier: An administrator without moderation or editorial permission receives the corresponding protected queue or action, an ordinary account receives overview data, or a restricted action succeeds when called directly.
