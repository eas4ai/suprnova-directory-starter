...
const form = useForm({ version: 0, slug: '', name: '', active: true })
...
function populate(item?: TaxonomyItem) {
  ...
  form.defaults({ version: item?.version ?? 0, slug: item?.slug ?? '', name: item?.name ?? '', active: item?.active ?? true }); form.reset(); form.clearErrors(); removal.clearErrors()
}
