...
<template>
  <section class="auth-card">
    ...
    <form class="space-y-4" @submit.prevent="form.post('/forgot-password', { onSuccess: () => sent = true })">
      ...
      <input id="email" v-model="form.email" type="email" autocomplete="email" required class="form-input" />
      <p v-if="form.errors.email" role="alert">{{ form.errors.email }}</p>
      <button type="submit" :disabled="form.processing" class="button button-primary">Send reset link</button>
    </form>
