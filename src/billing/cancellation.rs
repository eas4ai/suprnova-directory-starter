...
pub async fn request(actor: i64, id: &str, gateway: &dyn Gateway) -> Result<(), FrameworkError> {
    ...
    if !row.cancel_requested {
        crate::notifications::payment_event(&tx, id, &format!("purchase:{id}:{}:cancel-requested", row.version + 1), "Cancellation requested. It takes effect only after confirmation by the payment provider.").await?;
    }
